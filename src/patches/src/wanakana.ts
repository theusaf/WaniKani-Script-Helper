...
function isCharInRange(t = "", e: number, n: number) {
...
function isEqual(t: number, e: number) {
  return t === e || !(!numberIsNaN(t) || !numberIsNaN(e));
}
function areInputsEqual(t: number[], e: number[]) {
  if (t.length !== e.length) return !1;
  for (var n = 0; n < t.length; n++) if (!isEqual(t[n], e[n])) return !1;
  return !0;
}
function memoizeOne(func: (...args: any) => any, func2: (...args: any) => any) {
  function memoizer(..._: any) {
...
function find(object: any, e: any, n?: any) {
  for (n of object.keys()) if (dequal(n, e)) return n;
}
function dequal(object: any, object2: any) {
  var tmp1, tmp2, tmp3;
  if (object === object2) return !0;
  if (
    object &&
    object2 &&
    (tmp1 = object.constructor) === object2.constructor
  ) {
    if (tmp1 === Date) return object.getTime() === object2.getTime();
    if (tmp1 === RegExp) return object.toString() === object2.toString();
    if (tmp1 === Array) {
      if ((tmp2 = object.length) === object2.length)
        for (; tmp2-- && dequal(object[tmp2], object2[tmp2]); );
      return -1 === tmp2;
    }
    if (tmp1 === Set) {
      ...
      for (tmp2 of object) {
        if (
          (tmp3 = tmp2) &&
          "object" == typeof tmp3 &&
          !(tmp3 = find(object2, tmp3))
        )
          return !1;
        if (!object2.has(tmp3)) return !1;
      }
      return !0;
    }
    if (tmp1 === Map) {
      ...
      for (tmp2 of object) {
        if (
          ...
          return !1;
        if (!dequal(tmp2[1], object2.get(tmp3))) return !1;
      }
      return !0;
    }
    if (tmp1 === ArrayBuffer)
      (object = new Uint8Array(object)), (object2 = new Uint8Array(object2));
    else if (tmp1 === DataView) {
      if ((tmp2 = object.byteLength) === object2.byteLength)
        for (; tmp2-- && object.getInt8(tmp2) === object2.getInt8(tmp2); );
      return -1 === tmp2;
    }
    if (ArrayBuffer.isView(object)) {
      if ((tmp2 = object.byteLength) === object2.byteLength)
        for (; tmp2-- && (object as any)[tmp2] === object2[tmp2]; );
      return -1 === tmp2;
    }
    if (!tmp1 || "object" == typeof object) {
      for (tmp1 in ((tmp2 = 0), object)) {
        if (
          hasOwnProperty.call(object, tmp1) &&
          ++tmp2 &&
          !hasOwnProperty.call(object2, tmp1)
        )
          return !1;
        if (!(tmp1 in object2) || !dequal(object[tmp1], object2[tmp1]))
          return !1;
      }
      return Object.keys(object2).length === tmp2;
    }
  }
  return object != object && object2 != object2;
}
function applyMapping(in1: any, in2: any, flag: boolean) {
  function getOffset(obj: any, index: any) {
    if (void 0 !== obj[index])
      return Object.assign({ "": obj[""] + index }, obj[index]);
  }
  function copyMap(text: string, index: number): any {
  ...
  function executeMap(obj: any, text: string, start: number, end: number): any {
    if (!text)
      return flag || 1 === Object.keys(obj).length
        ? obj[""]
          ? [[start, end, obj[""]]]
          : []
        : [[start, end, null]];
    if (1 === Object.keys(obj).length)
      return [[start, end, obj[""]]].concat(copyMap(text, end));
    const offset = getOffset(obj, text.charAt(0));
    return void 0 === offset
      ? [[start, end, obj[""]]].concat(copyMap(text, end))
      : executeMap(offset, text.slice(1), start, end + 1);
  }
  const map2 = in2;
  return copyMap(in1, 0);
}
function transform(t: any): any {
  return Object.entries(t).reduce<any>((t, [e, n]) => {
    const a = "string" === typeOf(n);
    return (t[e] = a ? { "": n } : transform(n)), t;
  }, {});
}
function getSubTreeOf(initialValue: any, text: string) {
  return text
    .split("")
...
function createCustomMapping(t = {}) {
  ...
  return (
    ...
    function (t: any) {
      function combine(obj: any, obj2: any): any {
        return void 0 === obj || "string" === typeOf(obj)
          ? obj2
          : Object.entries(obj2).reduce(
              (prev, [key, val]) => (
                (prev[key] = combine(obj[key], val)), prev
              ),
              obj
            );
      }
      return combine(JSON.parse(JSON.stringify(t)), e);
    }
  );
}
function mergeCustomMapping(t: any, e: any) {
...
function makeOnInput(t: any) {
  ...
  return function ({ target: t }: { target: HTMLInputElement }) {
    t.value !== e &&
...
function trackListeners(t: string, e: any, n: any) {
  nt = nt.concat({ id: t, inputHandler: e, compositionHandler: n });
}
function untrackListeners({ id: t }: { id: string }) {
  nt = nt.filter(({ id: e }) => e !== t);
}
function findListeners(t: HTMLElement) {
  return t && nt.find(({ id: e }) => e === t.getAttribute("data-wanakana-id"));
}
function splitInput(t = "", e = 0, n: any[] = []) {
  let a, i, r;
  return (
    0 === e && n.includes(t[0])
...
function workFromStart(t: string, e: string[]) {
  return [
    "",
...
function takeWhileAndSlice(
  t: any[] | string = {} as any[],
  toBool = (t: any, ..._: any) => !!t
): [string, string[]] {
  const n = [],
    { length: a } = t;
  let i = 0;
  for (; i < a && toBool(t[i], i); ) n.push(t[i]), (i += 1);
  return [n.join(""), (t as any[]).slice(i)];
}
function bind(t: HTMLElement = {} as HTMLElement, e = {}, n = !1) {
  ...
  ].forEach((e: { name: string; value: string }) => {
    (r[e.name] = t.getAttribute(e.name)), t.setAttribute(e.name, e.value);
  }),
    (t.dataset.previousAttributes = JSON.stringify(r)),
    t.addEventListener("input", a as () => void),
    t.addEventListener("compositionupdate", onComposition as () => void),
    t.addEventListener("compositionend", onComposition as () => void),
    trackListeners(i, a, onComposition),
    !0 === n && addDebugListeners(t);
}
function unbind(t: HTMLElement, e = !1) {
...
function katakanaToHiragana(
  ...
  e: (a: string) => string,
  {
    isDestinationRomaji: n,
    convertLongVowelMark: a,
  }: {
    isDestinationRomaji?: boolean;
  ...
  return t
    ...
    .reduce((r, o, s) => {
      ...
      if (a && i && isCharInnerLongDash(o, s)) {
        const a = e(i).slice(-1) as keyof typeof it;
...
function getKanaToRomajiTree(t: string) {
  return t === ROMANIZATIONS.HEPBURN ? getKanaToHepburnTree() : {};
}
function createKanaToHepburnMap() {
  const hiraRomajiTransform = transform(hiraganaToRomajiMap),
    getHiraRomajiSubtree = (text: string) =>
      getSubTreeOf(hiraRomajiTransform, text),
    n = (t: string, n: string) => {
      getHiraRomajiSubtree(t)[""] = n;
    };
  return (
    ...
    iVowelList.forEach((t) => {
      const a = getHiraRomajiSubtree(t)[""][0];
    ...
    (hiraRomajiTransform["\u3063"] = resolveTsu(hiraRomajiTransform)),
    Object.entries(smallKanaMap).forEach(([t, e]) => {
      n(t, e);
    }),
    vowels.forEach((t) => {
      n(`\u3093${t}`, `n'${getHiraRomajiSubtree(t)[""]}`);
    }),
    Object.freeze(JSON.parse(JSON.stringify(hiraRomajiTransform)))
  );
}
function resolveTsu(t: any) {
  return Object.entries(t).reduce<any>((t, [e, n]) => {
    if (e) t[e] = resolveTsu(n);
    else {
      const a = (n as string).charAt(0) as keyof typeof validRomajiSet;
...
function toRomaji(t = "", e = {}, n?: any): string {
...
function getType(char: string, e = !1) {
  ...
  if (e)
    switch (!0) {
      case isCharJaNum(char):
      case isCharEnNum(char):
        return OTHER;
      case isCharEnSpace(char):
        return EN;
      case isCharEnglishPunctuation(char):
        return OTHER;
      case isCharJaSpace(char):
        return a;
      case isCharJapanesePunctuation(char):
        return OTHER;
      case isCharJapanese(char):
        return a;
      case isCharRomaji(char):
        return EN;
      default:
        return OTHER;
    }
    ...
    switch (!0) {
      ...
      case isCharJapanesePunctuation(char):
        return JA_PUNC;
      case isCharKanji(char):
        return KANJI;
      case isCharHiragana(char):
        return c;
      case isCharKatakana(char):
        return KATA;
      case isCharJapanese(char):
        return a;
      case isCharRomaji(char):
...
function tokenize(t: string, { compact: e = !1, detailed: n = !1 } = {}) {
...
export interface WanaKanaOptions extends Record<string, any> {
...
const mergeWithDefaultOptions = (t = {}): WanaKanaOptions =>
...
const onInput = ({
  ...
  addDebugListeners = (t: EventTarget) => {
    Object.entries(compositionEvents).forEach(([e, n]) =>
      t.addEventListener(e, n as EventListener)
    );
  },
  removeDebugListeners = (t: EventTarget) => {
    Object.entries(compositionEvents).forEach(([e, n]) =>
      t.removeEventListener(e, n as EventListener)
    );
  },
  ot = ["TEXTAREA", "INPUT"];
let rt = 0;
const newId = () => ((rt += 1), `${Date.now()}${rt}`),
  isCharInitialLongDash = (t: string, e: number) => isCharLongDash(t) && e < 1,
  isCharInnerLongDash = (t: string, e: number) => isCharLongDash(t) && e > 0,
  isKanaAsSymbol = (t: string) => ["\u30f6", "\u30f5"].includes(t),
...
const hiraganaToRomajiMap = {
  ...
  isLeadingWithoutInitialKana = (t: string, e: boolean) => e && !isKana(t[0]),
  isTrailingWithoutFinalKana = (t: string, e: boolean) =>
    !e && !isKana(t[t.length - 1]),
  isInvalidMatcher = (t: string, e: any) =>
